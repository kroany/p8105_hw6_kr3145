...
The distribution of the ratio of b1 and b2 is centered at around -180,
with a range between under -400 to around -100. In addition, the
distribution is left skewed, with a heavy tail extending to the lower
values.
...
The distribution of r.squared is relatively normally distributed and
centered at around 0.940, with a range between 0.92 to 0.95. This means
that a higher percentage of variation in tmax is explained by variation
in tmin and prcp in the model, demonstrating that the model with tmin
and prcp as predictors and tmax as the outcome is a good model. In
addition, the distribution has a slight tail extending to the lower
values.
...
important predictor to include in the regression model, since certain
race groups are known to have higher prevalence of low birthweight
babies. Therefore, my chosen final regression model will include
variables related to both the infant and the mother to produce the best
...
Based on the comparison between the three models, the 2nd model with
length and gestational age as main effects has the has the highest
average RMSE values compared to the 1st model and 3rd model,
demonstrating that the 2nd model is not the most optimal model to
predict infant birthweight. The 1st model using head circumference,
length, gestational weeks, smoking, and mother’s race as predictors and
the third model using head circumference, length, sex, and all
interactions as predictors are somewhat comparable in predicting
birthweight. However, the 1st model is still the most optimal model to
predict infant’s birthweight compared to models 2 and 3.
