...
distribution also has a heavy tail extending to the lower values.
...
The distribution of r.squared is centered at around 0.940, with a range
between 0.92 to 0.95. This means that a higher percentage of variation
in tmax is explained by variation in tmin and prcp in the model,
demonstrating that the model with tmin and prcp as predictors and tmax
as the outcome is a good model. In addition, the distribution has a
heavy tail extending to the lower values.
...
interactions as predictors are comparable in predicting birthweight.
